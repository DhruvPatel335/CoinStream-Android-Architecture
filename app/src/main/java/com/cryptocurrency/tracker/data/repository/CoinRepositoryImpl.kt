...
class CoinRepositoryImpl(
    ...
    override fun getCoins(): Flow<Resource<List<Coin>>> = flow {
        ...
        val localCoins = dao.get().getAllCoins().map { it.toCoin() }
        emit(Resource.Loading(data = localCoins))

        try {
            val remoteCoins = api.get().getCoins()
            // We might want to keep this for the non-paged parts of the app
            dao.get().deleteCoins()
            dao.get().insertCoins(remoteCoins.map { it.toCoinEntity() })
            val updatedLocalCoins = dao.get().getAllCoins().map { it.toCoin() }
    ...
    override fun observeCoinsPaged(): Flow<PagingData<Coin>> {
        return Pager(
            ...
            remoteMediator = CoinRemoteMediator(db, api.get()),
            pagingSourceFactory = { dao.get().observeAllCoinsPaged() }
        ).flow.map { pagingData ->
    ...
    override fun observeCoins(): Flow<List<Coin>> {
        ...
        return dao.get().observeAllCoins().map { entities ->
    ...
    override suspend fun getCoinById(id: String): Coin? {
        return dao.get().getCoinById(id)?.toCoin()
    }
}
